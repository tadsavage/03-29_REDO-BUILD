...
**Verification status:** compiled clean. Live-proved the core mechanism by invoking `SeekViaNavMesh` via reflection on a real in-scene `ReachTruckOperator` — it covered ~13m via genuine NavMeshAgent movement (not a teleport) in 3 real seconds, arrived within the MHE `stoppingDistance`, and cleanly handed back to manual control (`agentEnabled`/`navEnabled` both false afterward). **Did NOT get a clean live confirmation of the obstacle-avoidance behavior specifically** — a follow-up test (two points straddling a rack, same as the earlier round's successful static-query proof) got contaminated because the vehicle's own real `Update()`/task-claiming loop kept running concurrently with the manual reflection test and appears to have taken over the same vehicle mid-test. The underlying facts are independently confirmed (this rack's `NavMeshObstacle` genuinely carves a hole; a raw `NavMesh.CalculatePath` between straddling points genuinely detours around it), so the combined behavior is expected to work, but **needs an actual eyes-on watch of a live putaway/offload next session** rather than resting on the reflection proof alone. Full detail in `[[reachtruck-hybrid-navmesh-seekposition]]` memory.
