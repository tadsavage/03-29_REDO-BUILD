...
#### NavMesh: MHE/workers can no longer path under/through racks (2026-07-25)

**Root cause:** racks contribute zero NavMesh geometry (`NavMeshManager.BuildMarkups()` marks every "Racking"-category root `ignoreFromBuild = true`), but the floor tile underneath is a *separate* `PlacedObject` and was still emitted as a normal walkable box by `AddFloorNavMeshSources()` regardless of what's sitting on top of it. Net effect: a rack's own footprint was fully walkable NavMesh, so a forklift on general patrol (`AiNavigation.GoToRandomWaypoint()`, idle/task-transit driving) could path straight under a loaded rack.

**Fix:** `NavMeshManager.GetRackOccupiedCells()` (new) unions every "Racking" object's occupied cells via the existing `RackGridUtil.GetCells()` helper (root cell + rotated footprint offsets — the same logic `PlacementGrid.RebuildFromRegistry()`/`RackCollectionDetector` already rely on), and `AddFloorNavMeshSources()` now skips emitting a walkable source for any cell in that set (both the elevated-tile path and the bare-yard fallback path). Re-scans the registry once per bake — same cost as `BuildMarkups()`'s own existing rack-markup loop, not a per-frame cost. Applies uniformly to every `NavMeshSurface` (Human/MHE/Rat all share the same floor-source generation), so no agent type can walk through a rack's footprint, not just forklifts.

**Why this can't break putaway/replenishment:** `ReachTruckOperator.Commandeer()` (called at the top of both `PutawayRoutine` and `ReplenishRoutine`) disables the vehicle's `NavMeshAgent`/`AiNavigation` for the *entire* insert/retrieve sequence — every drive during a task (`DriveToPoint`, `DriveForksFirst`, `FaceForks`, `ReverseToPoint`) is direct `transform.position`/`transform.rotation` manipulation, never `NavMeshAgent.SetDestination`. The reach truck never consults the NavMesh once a task starts, so blocking a rack's footprint only affects idle/patrol driving between tasks. The aisle-side approach anchors (`LocApproachAnchor`, `ChepAnchorFront/Rear`) sit in the aisle cell, not the rack's own footprint cell, so they're unaffected either way.

**Not yet play-verified** — compiled clean (no `error CS` in console) but Unity hadn't picked up the recompile by the time this was written (Editor was in Play mode with focus elsewhere). Verify next session: place a rack, confirm the NavMesh visualization (Window → AI → Navigation, or the baked gizmo) shows a gap under it, and confirm a forklift's idle patrol routes around it instead of through it.
