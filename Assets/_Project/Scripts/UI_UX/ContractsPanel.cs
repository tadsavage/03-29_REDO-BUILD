...
public class ContractsPanel : IUIPanel
{
    ...
    private const float ModalWidth  = 980f;
    ...
    {
        _overlay = Build(out _cardScroll, out _footerMessage);
        root.Add(_overlay);
        Hide();
    }
    ...
    private VisualElement Build(out ScrollView cardScroll, out Label footerMessage)
    {
        ...
        var sub = new Label("Sign an account to bring work in. Standing accounts send orders every day; " +
                            "wholesale deals drop a full trailer once.");
        ApplyFont(sub, size: 13);
        ...
        ApplyFont(footerMessage, size: 13);
                                    ...
                                    InventoryService inv, bool taken, int rowIndex)
    {
        ...
        ApplyFont(value, bold: true, size: 19);
        value.style.color = new StyleColor(ColMoney);
        right.Add(value);

        var valueCaption = new Label(contract.IsWholesale ? "one-off value" : "per day, estimated");
        ApplyFont(valueCaption, size: 10);
        ...
        if (taken)
        {
            ...
            ApplyFont(signedLabel, bold: true, size: 13);
    ...
    /// <summary>
    /// Rough money the offer represents, for comparing cards. An ESTIMATE by construction — the real
    /// figure depends on which SKUs the roll picks — so it's prefixed with "~" rather than presented
    /// as a quote. Falls back to a pay-rate hint if inventory isn't available to price against.
    /// </summary>
