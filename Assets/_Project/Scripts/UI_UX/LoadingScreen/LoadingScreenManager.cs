...
public class LoadingScreenManager : MonoBehaviour
{
    ...
    // Fork pivot is (0.5, 0) — bottom-anchored — so this Y is the icon's bottom edge.
    // Bar bottom = BAR_Y - BAR_H/2 = -108, so the fork's bottom rests on the bar's bottom.
    [SerializeField] private float forkY = -108f;

    private const float TRACK_LEFT = -BAR_W * 0.5f;

    // Animation — serialized so you can tweak in Inspector without recompiling
    // Total minimum visible time = START_DELAY + FILL_DURATION = 0.25 + 3.0 = 3.25s
    [SerializeField] private float startDelay = 0.25f;   // pause before bar moves
    ...
    {
#if UNITY_EDITOR
        // Locate by asset name rather than a hard path so the icon survives folder moves.
        // Prefer the .png — there's also a ForkSprite.JPG, and JPEGs have no alpha channel
        // (loading it gives the fork an opaque background).
        string chosen = null;
        foreach (var guid in UnityEditor.AssetDatabase.FindAssets("ForkSprite t:Texture2D"))
        {
            string path = UnityEditor.AssetDatabase.GUIDToAssetPath(guid);
            if (path.EndsWith(".png", System.StringComparison.OrdinalIgnoreCase)) { chosen = path; break; }
            if (chosen == null) chosen = path;   // fallback if no .png is found
        }
        if (chosen != null)
        {
            var tex = UnityEditor.AssetDatabase.LoadAssetAtPath<Texture2D>(chosen);
