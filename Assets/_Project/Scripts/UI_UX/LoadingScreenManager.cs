...
public class LoadingScreenManager : MonoBehaviour
{
    ...
    {
#if UNITY_EDITOR
        // Found by name rather than a hardcoded path so a future folder move can't break it.
...
#if UNITY_EDITOR
        // Found by name rather than a hardcoded path so a future folder move can't break it.
        string[] guids = UnityEditor.AssetDatabase.FindAssets("ForkSprite t:Texture2D");
        if (guids.Length == 0) return null;
        var tex = UnityEditor.AssetDatabase.LoadAssetAtPath<Texture2D>(UnityEditor.AssetDatabase.GUIDToAssetPath(guids[0]));
