...
public class ToolsWindowController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        _tabPallet.clicked += () => SwitchTab("pallet");
        _tabDev.clicked    += () => SwitchTab("dev");

        // Titlebar drag → moves the whole window
        var titlebar = root.Q("tools-titlebar");
        titlebar.RegisterCallback<PointerDownEvent>(OnTitlebarDown);
        titlebar.RegisterCallback<PointerMoveEvent>(OnTitlebarMove);
        titlebar.RegisterCallback<PointerUpEvent>(OnTitlebarUp);
        ...
        root.Q<Button>("plt-build").clicked += OnPalletBuild;
    ...
    private void OnTitlebarDown(PointerDownEvent evt)
    {
        ...
        {
            var root = _doc.rootVisualElement;
            _windowStartPos.x = root.resolvedStyle.width - _window.resolvedStyle.width - _window.resolvedStyle.right;
        }
    ...
    private void OnTitlebarMove(PointerMoveEvent evt)
    {
        if (!_dragging) return;
    ...
    private void OnTitlebarUp(PointerUpEvent evt)
    {
        var titlebar = _doc.rootVisualElement.Q("tools-titlebar");
